function serverquery(){
    ...
    .then(data => {
        if (JSON.stringify(data.online, null, 2) =='true'){
        ...
        else{outputElementPlayerList.textContent ='未查询到列表'}
     }
     else {outputElementName.innerHTML='服务器离线';
        outputElementOnline.textContent ='✖回归虚无'
     }
    }
    )
    .catch(error => {
